...
const sections = mockData.terms.sections

export default function Terms() {
  const [activeSection, setActiveSection] = useState('acceptance')
  const [scrolled, setScrolled] = useState(false)
  const { plans, contactEmail } = mockData.terms
  const { serviceHighlights } = mockData.shared
    ...
    <div className="min-h-screen bg-[var(--color-background)] text-white">
      ...
      <div className="relative pt-32 pb-16 px-6 border-b border-[var(--color-card)]">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          {/* Alert banner */}
          <div className="mt-8 flex items-start gap-3 bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/20 rounded-xl p-4 max-w-2xl">
      ...
      <div className="mx-auto max-w-6xl px-6 py-16 flex gap-16">
        ...
        <aside className="hidden lg:block w-52 flex-shrink-0">
          <div className="sticky top-24">
            ...
            <div className="mt-10 p-4 rounded-xl bg-[var(--color-surface-deep)] border border-[var(--color-surface-2)]">
              <Shield className="h-4 w-4 text-[var(--color-accent)] mb-2" />
              <div className="text-xs text-white font-semibold mb-1">Questions?</div>
              <div className="text-[11px] text-gray-500 leading-relaxed">Email us at <span className="text-[var(--color-accent)]">{contactEmail}</span></div>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0 prose-section space-y-12">
          ...
          <section id="services" className="section-fade" style={{animationDelay:'0.15s'}}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 my-6">
              {serviceHighlights.map((s, i) => (
                <div key={i} className="bg-[var(--color-surface-deep)] border border-[var(--color-surface-2)] rounded-xl p-4 text-center hover:border-[var(--color-accent)]/20 transition-colors">
          ...
          <section id="payment" className="section-fade" style={{animationDelay:'0.25s'}}>
            ...
            <div className="bg-[var(--color-surface-deep)] border border-[var(--color-surface-2)] rounded-2xl overflow-hidden my-6">
              <div className="grid grid-cols-3 divide-x divide-[var(--color-surface-2)]">
                {plans.map((p, i) => (
          ...
          <section id="prohibited" className="section-fade" style={{animationDelay:'0.3s'}}>
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[var(--color-surface-2)]">
              <Ban className="h-5 w-5 text-[var(--color-accent)]" />
          ...
          <section id="disclaimer" className="section-fade" style={{animationDelay:'0.35s'}}>
            ...
            <div className="bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/15 rounded-xl p-5 my-4">
      ...
      <footer className="border-t border-[var(--color-card)] py-8 px-6">
        <div className="mx-auto max-w-6xl flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-5 text-xs text-gray-500">
            <Link to="/terms" className="text-[var(--color-accent)]">Terms of Service</Link>
